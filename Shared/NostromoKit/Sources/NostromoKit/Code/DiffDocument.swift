...
/// `macOS/Nostromo/UI/DiffDocument.swift` and this one lives here for iOS.
/// Deduplicating the two is deferred (memo B11) — see the PR body.
public struct DiffDocument: Equatable {
    ...
    public init(payload: DiffPayload) {
    ...
    /// are offsets into.
    public var text: String { rows.map(\.text).joined(separator: "\n") }

    /// The character range of `rows[index]`, excluding its terminator.
    ///
    /// UTF-16 code units, matching `NSRange`/`NSTextView`.
    public func characterRange(ofRow index: Int) -> NSRange? {
    ...
    public func characterRange(fromRow start: Int, toRow end: Int) -> NSRange? {
        guard start <= end,
    ...
    public func rowIndex(forPath path: String?, line: Int) -> Int? {
    ...
    public func rowIndices(forPath path: String?, from start: Int, to end: Int) -> [Int] {
