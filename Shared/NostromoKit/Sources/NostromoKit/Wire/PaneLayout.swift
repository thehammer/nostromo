...
public struct PrListItemModel: Decodable, Identifiable, Equatable {
    ...
    /// against `PrQueueItem.id`, etc.) — those must keep using `id` above.
    ///
    /// Folds `bucket` into `id` so that a PR moving between review-queue
    /// buckets (e.g. `requested` -> `needs_review`, see
    /// `src/data/perri_queue_native.rs` bucket precedence) changes this row's
...
extension PaneContentWire: Equatable {
    /// `.text`/`.loading`/`.error`/`.prList`/`.code`/`.diff`/`.prConversation`/
    /// `.ticket` all carry `Equatable` payloads and compare structurally.
    /// `.jsonSnapshot` and `.unknown` carry a bare `Any` payload, so they
    /// compare via `jsonPayloadsEqual` — structural equality through
    /// Foundation bridging — rather than always reporting "changed". Note
    /// this means `1` and `1.0` compare equal (NSNumber bridging); for a
