// Ported verbatim from `macOS/NostromoTests/DiffDocumentTests.swift`
// (ios-curated-view-parity W8, D1). Per the wedge's plan: "port them, watch
// them pass, and only then extend" — this file is the port, unmodified apart
// from the `@testable import NostromoKit` this target needs (macOS's copy is
// compiled directly into its test bundle with no host app and no
// `@testable import`; NostromoKit is a real module, so this target needs the
// import to see `DiffDocument`, which is `public` but not `open`/exported
// without it), the same adjustment W7's `CodeDocumentTests.swift` made for
// its own macOS source. Any test that had to change to pass would be a
// reported divergence in line addressing between the two clients, not an
// adjustment — none did.
...
/// curated-agent-views; ported to NostromoKit in ios-curated-view-parity W8).
