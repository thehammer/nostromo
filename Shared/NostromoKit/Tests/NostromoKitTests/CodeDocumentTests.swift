...
// Ported verbatim from `macOS/NostromoTests/CodeDocumentTests.swift`
// (ios-curated-view-parity W7). Per the wedge's plan: "port them, watch them
// pass, and only then extend" — this file is the port, unmodified apart from
// the `@testable import NostromoKit` this target needs (macOS's copy is
...
// import to see `CodeDocument`, which is `public` but not `open`/exported
// without it). Any test that had to change to pass would be a reported
// divergence in line addressing between the two clients, not an adjustment —
// none did.

/// Behavioural coverage for `CodeDocument` — the line/character arithmetic a
/// `code` pane's gutter, scroll, and emphasis all depend on, shared by iOS
/// and macOS (W2 — curated-agent-views; ported to NostromoKit in
